...
export class ProcessedMatchRepository {
  ...
  public async addProcessedMatch(matchId: string): Promise<void> {
    await this.prisma.processedMatch.create({ data: { matchId } });
  }

  public async removeProcessedMatch(matchId: string): Promise<boolean> {
    try {
      await this.prisma.processedMatch.delete({ where: { matchId } });
      return true;
    } catch (err: any) {
  ...
  public async removeLastProcessedMatch(): Promise<string | null> {
    const last = await this.prisma.processedMatch.findFirst({
      orderBy: { processedAt: 'desc' },
      select: { matchId: true },
    });
    if (!last) return null;
    await this.prisma.processedMatch.delete({ where: { matchId: last.matchId } });
    return last.matchId;
  }
