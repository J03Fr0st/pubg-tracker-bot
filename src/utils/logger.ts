...
export class Logger {
     ...
     * @param context Optional context or data
     */
    private log(level: LogLevel, message: string, icon: string, color: string, context?: string | object | Error): void {
        if (level < this.settings.level) {
            return;
        }

        const timestamp = this.settings.enableTimestamp ? this.formatTimestamp() : '';
        const coloredMessage = this.settings.enableColors ? `${color}${message}${this.colors.reset}` : message;
...
export const success = (message: string, context?: string | object) => logger.success(message, context);
export const warn = (message: string, context?: string | object) => logger.warn(message, context);
export const error = (message: string, error?: Error | string | object) => logger.error(message, error);

// Export specialized logging methods
export const api = (message: string, context?: string | object) => logger.api(message, context);
export const database = (message: string, context?: string | object) => logger.database(message, context);
export const discord = (message: string, context?: string | object) => logger.discord(message, context);
export const monitor = (message: string, context?: string | object) => logger.monitor(message, context);
export const logConfig = (message: string, context?: string | object) => logger.config(message, context);
export const startup = (message: string, context?: string | object) => logger.startup(message, context);
export const shutdown = (message: string, context?: string | object) => logger.shutdown(message, context); 